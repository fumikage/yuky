...
export default {
    ...
    actions: {
        ...
        async saveProfilPicture(context, { id, files }) {
            try {
                await userService.saveProfilPicture(id, files);
            } catch (error) {
                APIError.handle(error);
            }
        },
        async getProfilPicture(context, { id }) {
            try {
                let response = await userService.getProfilPicture(id);
                return response;
            } catch (error) {
                APIError.handle(error);
            }
